...
6. **Phase 3 reconciliation** — `contract-verify` in Phase 3 cross-checks the final `contract.json` (including `deviations[]`) against both frontend implementation and backend implementation. Deviations already recorded in `deviations[]` are recognized as intentional; only unrecorded drift surfaces as a `mismatch` finding. **Phase 3 special case**: if the user directs the agent to adjust the contract during Phase 3 (after contract-verify or even after verify-full), the agent MUST: (a) record the DEV entry in `contract.json.deviations[]` before making the change; (b) update `contract.json` and re-validate; (c) re-run `contract-verify` to confirm the new contract is consistent across frontend/backend/PRD-pointer; (d) if verify-full already passed, re-run verify-full for the affected scope; (e) record the deviation in `phase-3-integration-report.md` `## Downstream notes`.
