...
- `loops/specs/<task>/state.yaml` (`ac_change` field, when supersedes an already-consumed handoff)
...
Brainstorming is a conditional clarification skill before PLAN, not a mandatory step in every non-quick workflow. In the 4-phase pipeline (design-intake → frontend → backend → integration), brainstorming runs within Phase 0/1/2 as needed to resolve material ambiguity before writing-plans produces the spec.
