...
4. **Append terminal outcome** — append exactly one terminal PASSED/FAILED line to `iterations.log` for this attempt. The line MUST include a `red:` and `green:` marker citing the failing and passing test summary (one line each), e.g.: `attempt 3 PASSED red: "should render submit button" (1 failed) green: "should render submit button" (1 passed)`. This lightweight marker enables downstream review to confirm the red→green cycle actually occurred without bloating iterations.log with full test output. If the test passed immediately on first run (no red phase), record `red: (none — passed immediately; reconciled spec)` and cite the reconciliation per step 3 ("If a new feature test passes immediately... stop and reconcile the spec").
