param(
    ...
    [Parameter(Mandatory = $true)][ValidateSet('harness-pm','harness-engineering')][string]$ExpectedConsumer
)
...
if ([string]$manifest.producer -notmatch '^harness-(pm|engineering)$') { Fail "invalid producer: $($manifest.producer) (must be harness-pm|engineering)" }
...
if ($null -ne $contractPath) {
    ...
    if ($lines.Count -lt 3 -or $lines[0] -ne '---') { Fail 'contract missing YAML envelope' }
        ...
        if ($end -lt 2) { Fail 'contract envelope is not closed' }
            ...
            # manifest/contract scalar consistency
            ...
            # (uses -gt to allow same-day incremental iterations, e.g. PM-ENG-20260704-002 supersedes PM-ENG-20260704-001)
            $supersedes = $scalar['supersedes']
            # supersedes must match handoff_id pattern (e.g., PM-ENG-20260705-001) or be 'null'
            if ($supersedes -and $supersedes -ne 'null' -and $supersedes -notmatch '^[A-Z]+-[A-Z]+-[0-9]{8}-[0-9]{3}$') {
                Fail "supersedes must match handoff_id pattern (e.g., PM-ENG-20260705-001) or be null, got: $supersedes"
            }
            ...
            $isFamily = $true  # new architecture: always family mode (2 frameworks, each has clear upstream/downstream)
            if ($isFamily) {
