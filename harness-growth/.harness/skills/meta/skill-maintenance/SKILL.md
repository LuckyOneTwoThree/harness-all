...
- No empty directory placeholders (except .gitkeep)
- Reasonable reads/writes declarations
...
1. **Scan the actual skill directory**
   - Use Glob to scan all SKILL.md under the 9 module subdirectories:
     - `.harness/skills/strategy/*/`
     - `.harness/skills/experiment/*/`
     - `.harness/skills/content/*/`
     - `.harness/skills/seo/*/`
     - `.harness/skills/lifecycle/*/`
     - `.harness/skills/acquisition/*/`
     - `.harness/skills/monetization/*/`
     - `.harness/skills/analytics/*/`
...
2. **Parse INDEX.md**
   ...
   - Identify skills marked as "to be built" (do not treat as missing)

3. **Consistency comparison**

   | Check item | Pass criteria | Failure handling |
   |------------|---------------|------------------|
   | Registered vs actual | Every "built" skill listed in INDEX.md must have a corresponding SKILL.md | Mark "registered but not found" |
   | Actual vs registered | Every directory with a SKILL.md must be registered in INDEX.md | Mark "unregistered" |
   | Empty directory | Every subdirectory under strategy/experiment/content/seo/lifecycle/acquisition/monetization/analytics/review/meta must contain a SKILL.md or .gitkeep | Mark "empty directory" |
   | Duplicate skill | The same skill name must not appear in multiple module subdirectories or meta at the same time | Mark "duplicate" |
...
7. **Repair (only after user authorization)**
   - Empty directory → ask the user, then delete (except .gitkeep placeholders)
...
- Treating skills marked as "to be built" as missing
