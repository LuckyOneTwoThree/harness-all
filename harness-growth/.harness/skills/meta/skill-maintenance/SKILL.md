...
1. **扫描实际 skill 目录**
   - 用 Glob 扫描 9 个模块子目录下的所有 SKILL.md：
...
2. **解析 INDEX.md**
   ...
   - 提取工作流名称
   - 识别"待建设"标注的 skill（不视为缺失）

3. **一致性对比**

   | 检查项 | 通过标准 | 失败处理 |
   |--------|---------|---------|
   | 已注册 vs 实际存在 | INDEX.md 中列出的"已建设"skill 都必须有对应 SKILL.md | 标记"注册但未找到" |
   | 实际存在 vs 已注册 | 每个有 SKILL.md 的目录都必须在 INDEX.md 中注册 | 标记"未注册" |
   | 空目录 | strategy/experiment/content/seo/lifecycle/acquisition/monetization/analytics/review/meta 下每个子目录必须包含 SKILL.md 或 .gitkeep | 标记"空目录" |
   | 重复 skill | 同一 skill 名不能同时出现在多个模块子目录或 meta 中 | 标记"重复" |
...
4. **检查 SKILL.md frontmatter**
   ...
   对每个 workflow .md 检查是否包含：
   - `workflow_id:` 且与文件名对应
   - `name:` 且与文件名一致
   - `default_mode:` 且为 deep/standard/skip 之一
   - 缺少任意一项 → 标记"workflow frontmatter 不完整"
...
6. **输出健康报告**
   ...
   | 工作流 frontmatter | ✓ / ✗ | 6/6 有 workflow_id+name+default_mode |
   ```

7. **修复（仅在用户授权后）**
   - 空目录 → 询问用户后删除（.gitkeep 占位除外）
