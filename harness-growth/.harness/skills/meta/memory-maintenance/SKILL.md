...
- `loops/specs/<experiment>/iterations.log` 的长度

**不是：**
- 删除当前活跃实验的 state.yaml / spec.md / evidence.md
- 删除 `.git` 历史
- 压缩或加密文件
...
| `iterations.log`（每个实验） | 100 行 | 归档到 `archives/iterations-<experiment>-<date>.log` |
...
4. **拆分 knowledge-base.md**
   ...
   - 增长框架特有：实验结论归档时，保留"结论摘要表"，只归档详细记录

5. **归档 iterations.log**
   - 保留最近 20 行在原地
   - 其余内容追加写入 `memory/archives/iterations-<experiment>-<date>.log`
   - 不删除原文件（LOOP 需要它）
...
- 归档时不保留最近内容
- 删除 knowledge-base.md 中的实验结论摘要表（只归档详细记录）
