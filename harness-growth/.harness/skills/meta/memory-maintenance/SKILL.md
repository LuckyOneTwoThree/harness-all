...
- The length of `loops/specs/<experiment>/iterations.log`

**Does not:**
- Delete state.yaml / spec.md / evidence.md of currently active experiments
- Delete `.git` history
...
| `iterations.log` (per experiment) | 100 lines | Archive to `archives/iterations-<experiment>-<date>.log` |
...
4. **Split knowledge-base.md**
   ...
   - Growth framework specific: when archiving experiment conclusions, keep the "conclusion summary table" and archive only the detailed records

5. **Archive iterations.log**
   - Keep the most recent 20 lines in place
   - Append the rest to `memory/archives/iterations-<experiment>-<date>.log`
...
## Prohibitions
- Deleting state.yaml / spec.md / evidence.md of currently active experiments
...
- Deleting the experiment conclusion summary table in knowledge-base.md (archive only the detailed records)
