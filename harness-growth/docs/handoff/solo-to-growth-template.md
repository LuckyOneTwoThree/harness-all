...
> 本模板是 harness-growth 的**接收模板**，用于识别上游 harness-solo 产出的 `solo-to-growth.md` 格式。
> 实际产出方为 harness-solo（见 harness-solo/docs/handoff/solo-to-growth-template.md）。
> 本文件保留一份副本便于 harness-growth 的 Agent 对照字段，不直接参与流转。
