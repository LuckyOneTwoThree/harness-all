...
foreach ($framework in $frameworks) {
    ...
    $required = @('AGENTS.md', 'SOUL.md', 'constitution.md', 'install.sh', '.harness/VERSION', '.harness/loops/LOOP.md', '.harness/loops/STATE_PROTOCOL.md', '.harness/loops/state.schema.json', '.harness/rules/risk-model.md', '.harness/rules/handoff-protocol.md', '.harness/rules/handoff-manifest.schema.json', '.harness/rules/acceptance-id-protocol.md', '.harness/scripts/validate-handoff.ps1', '.harness/skills/INDEX.md')
...
if (@($stateProtocolHashes.Values | Select-Object -Unique).Count -ne 1) {
    Add-ValidationError 'STATE_PROTOCOL.md differs across frameworks'
}
if (@($stateSchemaHashes.Values | Select-Object -Unique).Count -ne 1) {
    Add-ValidationError 'state.schema.json differs across frameworks'
}
if (@($riskModelHashes.Values | Select-Object -Unique).Count -ne 1) {
    Add-ValidationError 'risk-model.md differs across frameworks'
}
if (@($handoffProtocolHashes.Values | Select-Object -Unique).Count -ne 1) { Add-ValidationError 'handoff-protocol.md differs across frameworks' }
...
if (@($acceptanceProtocolHashes.Values | Select-Object -Unique).Count -ne 1) { Add-ValidationError 'acceptance-id-protocol.md differs across frameworks' }
...
$producerConsumerContracts = @(
    ...
    @('harness-solo/.harness/skills/meta/session-end/SKILL.md', 'solo-to-ops.md', 'harness-ops/.harness/skills/meta/session-start/SKILL.md'),
    @('harness-growth/.harness/skills/meta/session-end/SKILL.md', 'growth-to-pm.md', 'harness-pm/.harness/skills/meta/session-start/SKILL.md')
)
...
foreach ($schema in @(
    ...
    'harness-solo/.harness/templates/component-bindings.example.json'
)) {
