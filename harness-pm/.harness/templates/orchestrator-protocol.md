...
- rules/security.md
- loops/LOOP.md
- templates/orchestrator-protocol.md (self-reference for orchestrator SKILL.md consumers)
...
|------|------|
| Schedule pipeline skills | Call sub-skills in phase order / parallel / conditional |
| Pass context | Pass the upstream skill's output as the downstream skill's input |
| Gate quality gates | Check gate conditions at the end of each phase; block or degrade if not passed |
...
- The orchestrator does not skip gate checks to enter the next phase directly

## 2. Orchestration modes

The orchestrator supports three orchestration modes, declared via `depends_on` in the Pipeline's `phases`:

### 2.1 Sequential orchestration

Phases have dependencies; the next executes only after the previous one completes.
...
Phases without dependencies can execute in parallel, used to accelerate collection-type tasks.
...
Decide whether to execute a phase based on upstream results or context, controlled via `gate.condition`.
...
phases:
  - id: phase-2
    ...
    gate:
      ...
      fail_action: "Skip this phase, label 'condition not met'"
```
...
The orchestrator passes context between phases via the following mechanisms:

| Pass method | Usage | Example |
|---------|------|------|
| File reference | Pass structured output across phases | `tam_som_ref: docs/discovery/market-analysis.md` |
| JSON data | Pass machine-consumed data across skills | `output/metrics/<id>.json` |
| Context variables | Share parameters within the same phase | `user_segment_data` |
...
|---------|---------|
| Single skill failure (parallel phase) | Do not block other skills in the same phase; mark the failed skill as "degraded execution" |
| Single skill failure (sequential phase) | Try the degraded plan to continue; if it cannot degrade, block the phase and request human intervention |
...
phases:
  # Phase 1: parallel collection (no dependencies)
  - id: phase-1
    name: "Parallel collection"
    depends_on: []
    skills:
  ...
  # Phase 2: integrated analysis (depends on phase 1)
  - id: phase-2
  ...
  # Phase 3: human approval (depends on phase 2)
  - id: phase-3
    name: "Human approval"
    depends_on: [phase-2]
    skills: [skill-d]
...
1. **When to use** — when this orchestrator should be invoked
2. **Inputs** — file dependencies (must include `templates/orchestrator-protocol.md`)
3. **Outputs** — files this orchestrator produces
4. **Core Principles / Orchestration Philosophy** — The orchestrator's design philosophy and key rules
5. **Responsibility Boundaries** (optional) — Explicit ✅/❌ lists of what the orchestrator does and does not do
...
13. **Downstream Connection** — Declaration of downstream orchestrators (see §6)
