...
| 市场规模数据 | JSON | ○ | docs/discovery/market-analysis.md（“市场规模”章节） | TAM/SAM/SOM与增长率 |
| 宏观环境数据 | JSON | ○ | docs/discovery/market-analysis.md（“PEST分析”章节） | PEST四维度趋势 |
...
**存储路径**：`docs/discovery/market-analysis.md（“竞品分析”章节）`
...
|---------------|-------------|---------|---------|
| 差异化策略变更 | prd-orchestrator | 变更的策略名称、调整方向、新优先级 | 策略新增/删除/优先级调整 |
| 竞争格局重大变化 | release-orchestrator | 格局变化描述、影响评估 | HHI指数跨阈值变化、核心竞品新增/退出 |
| 市场规模显著变化 | opportunity-definition | 新市场规模数据、变化原因 | TAM/SAM/SOM变化幅度>20% |
| 护城河评估变更 | insight-analysis | 竞品名称、旧等级→新等级 | 核心竞品护城河等级跨档变化 |
| 定位图空白区域变化 | prd-orchestrator | 空白区域变化描述 | 空白区域消失或新空白出现 |
