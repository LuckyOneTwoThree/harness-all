...
输出: docs/monitoring/release-notes.md（“验收报告”章节）
...
输出: docs/monitoring/release-notes.md（“发布检查清单”章节）
验证: 检查项全覆盖；所有阻断项已解决
模式: 🤖
```
...
输出: docs/monitoring/release-notes.md（“灰度计划”章节）
...
输出: docs/monitoring/release-notes.md（“发布说明”章节，覆盖）
...
所有子Skill执行完成后，必须生成阶段总结文档，写入 `output/phase-reports/release-orchestrator.json`，包含以下6项结构（均不可为空）：
...
| 总结输出路径 | output/phase-reports/release-orchestrator.json |
...
| 阶段总结已生成 | output/phase-reports/release-orchestrator.json 已生成且6项结构均非空 | 补充缺失结构项后重新生成 |
