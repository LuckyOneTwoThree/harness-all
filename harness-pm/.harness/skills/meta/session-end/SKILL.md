...
6. **Produce handoff documents** (optional, executed when conditions are met)
   ...
   **Write Access Unidirectional Isolation (Non-negotiable)**: Only the producer can write to a handoff document. `pm-to-solo.md` can only be written by PM, `pm-to-design.md` can only be written by PM, `pm-to-growth.md` can only be written by PM, and so on. Consumers can only read; modifying upstream handoff documents is prohibited. To provide feedback, use `AskUserQuestion` to relay through the user, or write to your own outbound handoff document.
   ...
   **Trigger conditions** (any one met):
   ...
   - The user explicitly requested "prepare handoff to growth"
   - Completed tasks involve metric definition, growth hypothesis, instrumentation plan, or other growth-consumed content

   **Output content** (fill in per pm-to-growth-template.md):
   - Product type / target audience / current phase
   - Positioning statement / North Star metric
   - OKR list / Persona growth characteristics
   - Growth hypothesis / existing data asset paths (if any)
