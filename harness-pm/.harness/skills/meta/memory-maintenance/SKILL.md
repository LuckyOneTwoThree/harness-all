...
4. **Trim knowledge-base.md**
   - For each section (`##` heading, e.g., "## Product decisions", "## Pitfalls", "## User insights"), keep the most recent N entries (default: 20 per section)
   ...
   - Rationale: sections (decisions / pitfalls / insights) have different lifecycles; splitting them into separate files breaks the "one knowledge base" mental model and reduces session-start load value
