...
4. **Read the feature board**
   ...
   - **Cross-framework reconciliation**: if a `solo-to-pm.md` handoff has been consumed (receipt exists in `docs/handoff/receipts/`), cross-check Solo's reported "Implemented Features" status against PM's `FEATURES.md`. For each feature Solo reports as `done`, PM's status should be at least `developing`. If PM shows `approved` while Solo reports `done`, flag the drift to the user: "Feature F-XXX is done in Solo but still 'approved' in PM — advance to 'developing' or 'launched'?". See DOMAIN_BOUNDARIES.md "FEATURES.md Cross-Framework Reconciliation" for the full status mapping.
...
5. **Check handoff documents** (from other harness family members)
   ...
   - If `solo-to-pm.md` exists (from harness-solo), validate it, report its feedback items, and route accepted consumption to prd-orchestrator phase 0 Branch B (engineering feedback triage). Apply batch-aware detection (see step 5a below).
   - Engineering feedback may include: implemented features, technical constraints, open issues, design issues (for secondary routing to harness-design).
   - If `design-to-pm.md` exists, validate it, report its feedback IDs, and route accepted consumption to prd-orchestrator phase 0 Branch A (design feedback triage).
...
### 5a. Batch-aware Detection (for solo-to-pm and design-to-pm with batch field)

When a newly accepted `solo-to-pm.md` or `design-to-pm.md` supersedes a previously consumed handoff and contains a `batch` field, use batch-aware detection to identify change scope:
...
2. **Incremental batch** (`batch.type: incremental`): use the `batch` field as the primary signal:
   ...
   - `batch.modified_acs` → feedback content changed (same AC ID; re-triage needed). Note: for reverse feedback channels (solo-to-pm, design-to-pm), modified_acs contains SAME AC IDs with changed feedback content — NOT new replacement IDs. See HANDOFF_PROTOCOL.md "Forward vs Reverse batch semantics".
