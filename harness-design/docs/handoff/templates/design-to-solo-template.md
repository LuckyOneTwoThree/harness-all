...
> - `ac_ids` envelope field MUST contain ALL valid AC/DAC IDs (unchanged + added + modified), never just the changed subset — this prevents AC/DAC loss if a previous design-to-solo was never consumed.
> - ACs/DACs marked `[unchanged]` may use a one-line summary + reference to the prior handoff (e.g., "see DESIGN-SOLO-20260704-001").
> - ACs/DACs marked `[added]` or `[modified]` MUST include the full description.
> - ACs/DACs marked `[superseded]` MUST include a pointer to the replacement ID; superseded IDs do NOT appear in `ac_ids` (only replacements do).
...
|-------|--------|-------------|
| AC-F01-001 | [added] | <stable testable description reused from PRD> |
| AC-F01-002 | [added] | <stable testable description reused from PRD> |
| AC-F02-001 | [added] | <stable testable description reused from PRD> |

### Design-derived DAC-xxx (new in this handoff)

| DAC ID | Change | Description |
|--------|--------|-------------|
| DAC-P01-001 | [added] | <stable page-scoped design criterion, e.g., "button hover state has a 200ms transition animation"> |
| DAC-GLOBAL-001 | [added] | <stable cross-page design criterion, e.g., "first screen LCP <= 2.5s"> |

> For incremental batches, the DAC table would look like:
>
> | DAC ID | Change | Description |
> |--------|--------|-------------|
> | DAC-P01-001 | [unchanged] | (see DESIGN-SOLO-20260704-001) button hover 200ms transition |
> | DAC-P01-002 | [modified] | <full new description> (replaces DAC-P01-003) |
> | DAC-P01-003 | [superseded] | superseded by DAC-P01-002 — hover cancel behavior refined |
> | DAC-P02-001 | [added] | <full description for new page design> |
...
> Full review evidence (pass/fail per dimension + critical findings): `loops/specs/<product-task>/review-evidence.md` (`## Product-Level Review` section)
