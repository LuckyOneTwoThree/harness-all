...
[![Skills](https://img.shields.io/badge/skills-16-red.svg)](#core-features)
...
> For product research / engineering development, see other members of the harness family; hand off via `docs/handoff/`.
...
### 8 Workflows Cover the Full Design Lifecycle

```
design-onboarding (first-time onboarding) → new-product-design (product-level) / new-design/design-iteration/design-system-setup/redesign (single-page) / image-to-contract (A4) → design-handoff (delivery)
```

| Workflow | Scenario | LOOP Type | Iteration Limit |
|--------|------|---------|:---:|
| design-onboarding | First-time onboarding, quick design system skeleton | No LOOP | - |
| new-product-design | Product-level design (plan all pages + pre-gen specs → per-page LOOPs → cross-page review (product-design-review) → unified design-review) | wireframe/ui-design/component | 5/5/5 |
| new-design | New design task (from 0 to 1, single page) | wireframe/ui-design | 5/5 |
| design-iteration | Design iteration (existing design optimization) | ui-design (A1: combined visual+interaction) | 5 |
| design-system-setup | Full design system build (with component LOOP + review) | component | 5 |
| redesign | Redesign (major revamp) | ui-design (A1: combined visual+interaction, mandatory) | 5 |
| image-to-contract | A4: extract component contract from existing design image | component | 5 |
| design-handoff | Design handoff | No LOOP | - |

### 16 Skills (12 design + 4 meta)
...
- Design output: ui-design (A1: combined visual + interaction) / wireframe / component-design
...
```
session-start → design-brief (hard gate) → PLAN → [conditional] LOOP(wireframe→verify) → LOOP(ui-design→verify) → design-review (includes a11y audit) → session-end
```
...
|------|---------|:---:|
| ui-design (A1 default) | Combined visual + interaction design task | 5 |
...
|--------|:---:|---------|
| new-design | 5/5 (each of the 2 LOOPs counts separately; A1 merged visual+interaction into ui-design) | Request human intervention |
...
|-----------|---------|
| "Make a nice-looking button" | "AC-F01-001: button has 4 states; annotate size + color + radius" |
| "This page should be modern" | "AC-F01-001: page uses 12-column grid / AC-F01-002: primary color #xxx / AC-F01-003: 8px spacing baseline" |
