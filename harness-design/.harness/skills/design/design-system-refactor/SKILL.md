...
- `docs/ui/*.md` (A1: combined visual + interaction design; component usage + states)
...
> **Boundary**: design-system-refactor owns `DESIGN.md` + `tokens.json` only. `docs/handoff/component-contract.json` is owned by design-handoff-spec; do not write it here. After DESIGN.md/tokens.json change, design-handoff-spec (or the verify skill) is responsible for propagating token references and bumping the component-contract revision.
...
- [ ] verify's lint step passes after refactoring (evidence: review-evidence.md `## Lint Report` section has no errors)
- [ ] DESIGN.md + tokens.json updated with matching revision/hash evidence (component-contract.json propagation is owned by design-handoff-spec / verify, not this skill)

## Relationship with LOOP

- Not run inside LOOP (runs in the post-LOOP design system maintenance stage)
- Reads LOOP-produced docs/ui/ as scan input (A1: combined visual + interaction)
