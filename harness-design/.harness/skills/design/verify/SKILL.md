...
- Design mockups to be checked (`docs/ui/<page>.md` — A1: combined visual + interaction; formerly `docs/visual/<page>.md`)

## Outputs
- loops/specs/<task>/review-evidence.md (verify + lint sections; design-review appends the rest)
- loops/specs/<task>/state.yaml
...
```
AC-F01-001: Login form contains email + password + submit button → ✓
AC-F01-002: Submit button has 4 states → ✓
AC-F01-003: No overflow at mobile 375px → ✗ (password input overflows)
```
...
//   node loops/specs/<product-task>/lint-all.mjs docs/visual/<page>.md
//   (lint output is captured into review-evidence.md `## Lint Report` section, not a separate file)
//
...
// Output to stdout for capture into review-evidence.md (no separate lint-report.md)
...
### 7. Write review-evidence.md (verify + lint sections)

verify writes the `## Verify Evidence` and `## Lint Report` sections. design-review later appends `## Five-Axis Review`, `## WCAG Audit`, `## Doubt-Driven Review` sections to the same file.
...
- AC-F01-001: ✓
- AC-F01-002: ✓
- AC-F01-003: ✗ (password input overflows)
...
## Conclusion (verify stage)
- [ ] Pass → proceed to next LOOP or out-of-LOOP gate
- [x] Fail → return to DESIGN stage
- Failure reason: AC-F01-003 not satisfied, 375px password input overflows / L001: 2 hardcoded hex colors
```
...
last_error: "AC-F01-003 not satisfied; L001: 2 hardcoded hex"  # or "" (on pass)
...
```
[<timestamp>] iter=<N> stage=design → verify FAILED: AC-F01-003 not satisfied; L001: 2 hardcoded hex colors
```
...
- Not writing review-evidence.md (verify + lint sections)
...
- [ ] AC annotated ✓/✗ item by item (evidence: review-evidence.md `## Verify Evidence` contains AC list)
- [ ] Constitution compliance check (evidence: review-evidence.md `## Verify Evidence` contains constitution check)
- [ ] Quick accessibility check (evidence: review-evidence.md `## Verify Evidence` contains contrast/keyboard)
...
- [ ] Lint output captured into review-evidence.md `## Lint Report` section (evidence: section exists with errors/warnings/info)
...
- [ ] review-evidence.md written with verify + lint sections (evidence: file exists with both sections)
...
- Loop type: applies to all loop types (ui-design / wireframe / component; A1: visual-design + interaction-design deprecated, merged into ui-design)
