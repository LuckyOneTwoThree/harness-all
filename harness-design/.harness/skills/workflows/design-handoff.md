...
- [ ] verify inside the LOOP passed (evidence: `loops/specs/<task>/review-evidence.md` `## Lint Report` section has no errors)
- [ ] design-review outside the LOOP passed (evidence: `loops/specs/<task>/review-evidence.md` contains a "passed" conclusion)
- [ ] design-review's Axis 5 accessibility audit passed (evidence: `loops/specs/<task>/review-evidence.md` `## WCAG Audit` section has no Critical failures)
...
- Read `loops/specs/<task>/review-evidence.md` `## Lint Report` section to confirm no error-level violations
- Read `loops/specs/<task>/review-evidence.md` to confirm the design-review conclusion is "passed"
- Read `loops/specs/<task>/review-evidence.md` `## WCAG Audit` section to confirm no Critical failures
...
- **Per-page tasks**: for each per-page task `<product-task>-<page-name>` listed in `docs/visual/DESIGN_PLAN.md` Section 2 (Status = done), verify:
  - `loops/specs/<product-task>-<page-name>/review-evidence.md` `## Lint Report` section has no error-level violations
  - `loops/specs/<product-task>-<page-name>/review-evidence.md` contains a "passed" design-review conclusion
...
- **Product-task**: verify the product-level consistency gate passed:
  - `loops/specs/<product-task>/review-evidence.md` `## Product-Level Review` section contains a "passed" conclusion with no open Critical findings
  - `loops/specs/<product-task>/state.yaml` status is not failed
- **Accessibility**: `loops/specs/<product-task>/review-evidence.md` `## WCAG Audit` section has no Critical failures (product-level review-evidence.md covers all pages in product-level mode)
