...
|------|------------------|----------------|----------------|
| **ui-design** (A1, default) | Combined visual + interaction design tasks | 5 | verify (incl. lint) pass + AC satisfied |
...
**A1 rationale (2026-07-05 audit)**: The former visual-design + interaction-design two-LOOP split caused a "split-then-merge" round-trip — design produced two separate `<page>.md` files per discipline, then handoff aggregated them back into `component-spec.md`. solo's `frontend-implementation` consumes `component-contract.json` (visual `token_refs` + interaction `states` merged), so the disciplinary split served no downstream consumer. The combined `ui-design` LOOP preserves the "structure → visual → interaction" stage order within a single LOOP (visual passes before interaction sub-stage begins), but verify runs once per iteration covering both, and output is a single `docs/ui/<page>.md`.

**Removal notes**:
- `prototype` changed to `wireframe`, semantics more accurate (the output is low-fidelity wireframes, not interactive prototypes)
- `redesign` removed (redesign is a workflow scenario; internally it still uses the visual-design loop → now uses ui-design loop)
- `visual-design` + `interaction-design` merged into `ui-design` (A1: avoid split-then-merge round-trip with solo's component-contract consumption pattern)
...
**Semantic notes**:
- "Single LOOP" refers to the iteration count within the same loop type (ui-design / wireframe / component)
...
Evidence from each VERIFY/LINT stage and the final design-review is consolidated into a single `loops/specs/<task>/review-evidence.md` (per A2: collapse former `evidence.md` + `lint-report.md` into one file with sections, to cut per-task file count from 5 to 3 and remove the verify→review handoff of two separate evidence files).

**File write semantics (A2 consolidated)**:

| File | Write Semantics | Reason | Operation |
|------|-----------------|--------|-----------|
| `spec.md` | Overwrite | Keep only the final passed spec | Write directly overwrites |
| `state.yaml` | Overwrite | Keep only the current state | Write directly overwrites; carries `spec_ref: loops/specs/<task>/spec.md` pointer (spec content is NOT inlined — state.yaml stays lean for circuit-breaker Read-on-VERIFY) |
| `review-evidence.md` | Overwrite | Keep only the final successful evidence (verify + lint + review in one file, sectioned) | Write directly overwrites |
| `iterations.log` | **Append** | Preserve complete iteration history | Read+concatenate+Write, or `echo >>` |

```
loops/specs/001-login-page/
├── spec.md              ← Design spec (overwrite: final passed version; AC list source of truth)
├── state.yaml           ← Loop state (overwrite: current state + spec_ref pointer; circuit-breaker lean read)
├── review-evidence.md   ← Consolidated evidence (overwrite: final success; sections = verify / lint / five-axis / wcag)
└── iterations.log       ← Iteration history (append-only, full trajectory)
```

**Why consolidated (A2 rationale)**:
- Former 5 files (`spec.md` / `state.yaml` / `evidence.md` / `lint-report.md` / `iterations.log`) → 4 files. `evidence.md` + `lint-report.md` merged into `review-evidence.md` (sectioned: `## Verify Evidence`, `## Lint Report`, `## Five-Axis Review`, `## WCAG Audit`).
- `spec.md` kept independent (NOT inlined into state.yaml): state.yaml is the circuit-breaker read target on every VERIFY; inlining spec would bloat every Read. `state.yaml` carries only a `spec_ref` pointer.
- All artifacts of one task in one directory → good cohesion, no cross-directory lookup.

**review-evidence.md section layout**:
```markdown
# Review Evidence — <task>

## Verify Evidence (Iteration <N>)
<AC item-by-item / constitution / quick a11y / deliverability>

## Lint Report
<errors / warnings / info from L001-L015>

## Five-Axis Review
<Axis 1-4 findings, per axis>

## WCAG 2.1 AA Static Audit (Axis 5)
<contrast / keyboard / screen reader / responsive / reduced-motion / dark mode>

## Doubt-Driven Review
<Critical findings + Nit/FYI>
...
```
[2026-06-20 14:30] iter=1 stage=design → verify FAILED: AC-F01-002 not satisfied (missing hover state)
...
Why designed this way?
  - spec.md kept independent → state.yaml stays lean for circuit-breaker Read; spec is the AC source of truth read by verify + design-review
  - review-evidence.md sectioned → verify writes `## Verify Evidence` + `## Lint Report`; design-review appends `## Five-Axis Review` + `## WCAG Audit` + `## Doubt-Driven Review`; single file, no cross-file handoff
...
substage: "<sub-stage description>"        # e.g., "visual" / "interaction" / "wireframe" / "component" (A5: memory-only between checkpoints, persisted at next mandatory write)
...
# Optional field (A2: spec pointer — spec.md kept independent, not inlined)
spec_ref: "loops/specs/<task>/spec.md"     # Pointer to the independent spec.md file; state.yaml stays lean for circuit-breaker Read
```

**stage enum values**:

| Value | Meaning | Write Timing (A5: mandatory checkpoint) |
|-------|---------|--------------|
| `plan` | Planning stage | Checkpoint 1: PLAN init (mandatory write) |
| `design` | Design stage | Checkpoint 2: DESIGN success (mandatory write — iteration increment persists here before VERIFY; A1: ui-design combined visual+interaction) |
| `verify` | Verification stage | Checkpoint 3: VERIFY/LINT complete (mandatory write — circuit breaker reads raw file here) |
| `lint` | Lint stage | Checkpoint 3: VERIFY/LINT complete (mandatory write — same as verify) |
| `review` | Final review stage (outside LOOP) | Checkpoint 4: review complete (mandatory write) |
...
- [ ] All inside-LOOP verify passed (each AC-xxx ✓) — evidence in `review-evidence.md` `## Verify Evidence` section
- [ ] All inside-LOOP verify lint step passed (no error-level violations) — evidence in `review-evidence.md` `## Lint Report` section
- [ ] Outside-LOOP design-review passed (Five-Axis + Doubt-Driven) — evidence in `review-evidence.md` `## Five-Axis Review` + `## Doubt-Driven Review` sections
- [ ] Outside-LOOP design-review Axis 5 passed (WCAG 2.1 AA static-checkable subset) — evidence in `review-evidence.md` `## WCAG Audit` section
