...
|:----:|------|------|:----:|:------:|:---------:|
| **harness-pm** | 核心 | 产品 · 战略 · 市场 · PRD · API 契约 · 指标 | ✅ 已构建 | 84 | 11 |
| **harness-engineering** | 核心 | 软件工程交付（4 阶段：design-intake · frontend · backend · integration） | ✅ 已构建 | 25 | 9 |
...
4 阶段工程交付：design-intake → frontend → backend → integration。25 个 skill（21 domain + 4 meta）+ 9 个 workflow。这是家族的**软件工程**成员——其他领域类型（数据、QA、安全等）以独立包加入，而不是作为 engineering 内部的 skill。
