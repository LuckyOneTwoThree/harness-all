...
> **一套"独立优先，契约协作"的 AI Agent 框架家族**
> 每个框架专注一个领域，独立工作；通过契约文档协作，形成完整闭环。
...
|:----:|------|:-----:|:--------:|:----:|
| **harness-pm** | 战略 · 市场 · 产品 · PRD · 指标 | 86 | 10 | ✅ |
...
| 维度 | 统一框架 | 独立框架（本方案） |
|------|---------|-------------------|
| 上下文成本 | 单 Agent 加载所有 skill，上下文爆炸 | 每个 Agent 只加载自己领域的 skill |
| 记忆污染 | 产品/工程/设计/增长记忆混在一起 | 每个框架独立记忆，互不干扰 |
| 调试隔离 | 一个领域的 bug 影响全局 | 框架间完全隔离 |
| 工具适配 | 一套工具链适配所有场景 | 每个框架按需选择工具 |
| 项目归属 | 一个项目一个 Agent | 不同框架可挂载到不同项目/工作目录 |

**结论**：上下文爆炸和记忆污染是 AI Agent 协作的核心痛点，独立框架是当前最务实的选择。

### 四条铁律

1. **独立自足** — 每个框架必须能独立完成自己领域的工作，不依赖其他框架
2. **契约协作** — 框架间通过 `docs/handoff/` 下的契约文档传递需求
3. **循环验证** — 所有任务走 LOOP（计划→执行→验证），证据驱动
...
大多数人用 AI Agent 是这样的：打开对话 → 描述需求 → 得到结果 → 关闭对话。下次再打开，一切从零开始。

```
❌ 没有框架的日常

  第 1 次对话：  "帮我写个 PRD"        → Agent 不知道你的产品背景，从头问起
  第 2 次对话：  "帮我设计这个页面"    → Agent 不知道 PRD 长什么样，从头问起
  第 3 次对话：  "帮我实现这个功能"    → Agent 不知道设计长什么样，从头问起
  第 4 次对话：  "帮我写个增长方案"    → Agent 不知道产品现状，从头问起
  ...每次对话都是失忆的，每次都要重建上下文
```

**核心问题不是 Agent 不够聪明，而是没有持久化的项目记忆和领域知识。**

### 框架解决了什么

harness 框架的本质不是"更好的提示词"，而是**为 AI Agent 构建持久化的项目知识体系**：

| 能力 | 无框架 | 有 harness 框架 |
|------|---------------|---------------------|
| **项目知识库** | 每次对话重新解释项目背景 | `knowledge-base.md` 持续积累项目决策、技术选型、踩坑经验 |
| **工作区记忆** | 关闭对话就遗忘 | `progress.md` 跨会话保持进度，下次自动恢复 |
| **领域专精** | 一个 Agent 什么都做，什么都做不好 | 每个框架专注一个领域，skill 精准匹配场景 |
| **上下文效率** | 全量加载，token 浪费严重 | INDEX → SKILL.md 按需加载，只读必要的 |
| **可追溯协作** | 口头/聊天记录传递需求 | 契约文档（handoff）结构化传递，AC 编号跨框架对齐 |
| **可验证质量** | "我觉得做完了" | LOOP 引擎 + 证据门禁，没有证据不能宣称完成 |
| **安全红线** | Agent 可能越界 | constitution.md 不可协商 + security.md 统一禁令 |
| **可复用经验** | 每个项目从零开始 | 模板 + 知识库 + skill 跨项目复用 |
...
│  → 明确 Agent 行为边界，不越界、不跑偏                       │
│  → 不同框架有不同的人格和原则，专精而非通用                   │
│  → 清晰的规则优先级：SOUL > AGENTS > rules > 对话 > 外部文件 │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
| 维度 | 纯提示词工程 | harness 框架 |
|------|----------------------|-------------------|
| 知识持久化 | 无（每次对话从零开始） | knowledge-base.md 跨会话积累 |
| 上下文恢复 | 无（手动复制粘贴） | progress.md + session-start 自动恢复 |
| 领域专精 | 靠提示词角色描述（脆弱） | SOUL.md + 82 个 PM skill 精准匹配 |
| 质量保障 | 人工检查 | LOOP 循环 + 证据门禁 + verify skill |
| 协作交接 | 复制粘贴聊天记录 | 契约文档结构化传递 + AC 编号对齐 |
| 安全边界 | 靠提示词约束（可被覆盖） | constitution.md 不可协商 + security.md |
| 经验复用 | 每个项目重写提示词 | 模板 + skill + 知识库跨项目复用 |
| 可扩展性 | 提示词越来越长，难以维护 | skill 模块化，INDEX 按需加载 |

**一句话总结**：提示词工程是"教 Agent 做一件事"；框架是"让 Agent 拥有持久的项目记忆和领域专长，越用越强"。
...
❌ 没有框架：
  ...
  你：（花 10 分钟重新解释）
  ...每次切换领域都要重建上下文，知识无法积累

✅ 有 harness 框架：

  你：启动 harness-pm，开始 new-product 工作流
  Agent：（自动读取 knowledge-base，了解项目背景）
  Agent：（执行 new-product 工作流，产出 PRD + 契约文档）
...
- **个人 AI 工作室**：一人 + 多个 AI Agent，每个 Agent 专精一个领域
- **小团队协作**：不同成员拥有不同框架，通过契约文档对齐
- **多项目并行**：每个框架可挂载到不同项目目录，互不干扰
...
| 类别 | 框架 | 职责 | 状态 | Skill 数 |
|------|------|------|------|---------|
| 核心 | **harness-pm** | 战略 · 市场 · 产品 · PRD · 指标 · 增长监控 | ✅ 已建成 | 86 skill（82 领域 + 4 元）+ 10 workflow |
| 核心 | **harness-design** | UI · 视觉 · 交互 · 原型 · 设计系统 | ✅ 已建成 | 18 skill（14 领域 + 4 元）+ 6 workflow |
| 核心 | **harness-solo** | 工程 · TDD · 调试 · 重构 · 验证 | ✅ 已建成 | 20 skill（16 领域 + 4 元）+ 7 workflow |
| 核心 | **harness-growth** | 运营 · 内容 · SEO · 增长实验 | ✅ 已建成 | 40 skill（36 领域 + 4 元）+ 6 workflow |
| 扩展 | **harness-ops** | 运维 · 部署 · 监控 · 容灾 | ✅ 已建成 | 32 skill（28 领域 + 4 元）+ 7 workflow |
| 扩展 | harness-data | 数据管道 · ETL · 指标生产 | 📋 P1 待建 | - |
| 扩展 | harness-qa | 质量保障 · 自动化测试 · 性能测试 | ⚠️ P2 按需 | - |
| 扩展 | harness-security | 安全审计 · 合规 · 渗透测试 | ⚠️ P3 按需 | - |
...
| 视觉设计（配色/排版/组件） | harness-design | pm 不指定配色，solo 不硬编码数值 |
...
| 运维保障（发布/监控/容灾） | harness-ops | solo 不直接操作生产环境，必须走 ops IaC |
...
每个框架是一个完全独立的目录，可挂载到不同项目目录，被不同 Agent 实例加载，拥有独立的 `.harness/` 配置、记忆、进度。

### 2. 契约协作 · 跨框架对齐

通过 `docs/handoff/` 下的契约文档传递需求，支持多人 + 多 Agent 协作。AC 编号系统跨框架对齐（`AC-xxx` 工程 + `DAC-xxx` 设计流入工程）。

### 3. LOOP 引擎 · 证据驱动

所有框架统一循环引擎，支持 state.yaml 断点续传、迭代上限保护、证据驱动原则。单一 LOOP 硬熔断 10 次迭代。

### 4. 安全红线 · 不可协商

每个框架的 `constitution.md` 定义不可协商原则；`rules/security.md` 定义统一禁令。ops 框架额外增加严格的 Secret 隔离和 7 层纵深防御。

### 5. 跨平台兼容

Agent 工具优先（Read/Write/Edit/Glob/Grep），bash 可选兜底。所有脚本都有 bash 可用性检查，Windows 下自动跳过。

### 6. 半自动化架构（harness-ops 专属）

Agent 提议 + 人类审批 + GitOps 执行。四种操作原语（inspect/propose/mutate-staging/mutate-prod）精准控制自动化边界。
...
git clone <harness-solo-repo>    # 工程实现
...
```
# 在项目目录启动 AI Agent（如 Trae IDE）

# Agent 会按加载链自动读取：
# 1. AGENTS.md（启动时强制读取）
# 2. SOUL.md + constitution.md（首次交互时）
# 3. skills/INDEX.md（选择 Skill 时）
# 4. 对应的 SKILL.md（执行任务时）
...
```bash
# 场景：PM 产出 PRD → Design 产出设计稿 → Solo 实现代码

# 1. 在产品项目目录使用 harness-pm
cd product-project
...
> **定位**："做对的事" — 产品探索、市场分析、PRD 生成、指标运营

**四原则**：
1. 探索先行 — 不假设需求，让调研数据说话
2. 契约驱动 — PRD 驱动设计，定位驱动品牌
3. 数据驱动 — 用数据减少猜测；决策权留给人类
4. 闭环迭代 — 指标 → 监控 → 迭代 → 反馈

**Skill 体系**（86 skill = 82 领域 + 4 元）：
- 模块 1 发现（12）：user-research / market
- 模块 2 商业战略（13）：business / planning
- 模块 3 构思与设计（9）：prd / validation
- 模块 4 指标设计（4）：metrics
- 模块 5 指标运营（11）：analysis / decision / experiment
- 模块 6 增长运营（16）：growth / acquisition / activation / retention / revenue
- 模块 7 监控与迭代（17）：monitoring / diagnosis / iteration / release

**特色机制**：
- **UI 指令越界门禁**：强制拦截 PM 在 PRD 输出时夹带具体视觉/交互形式
- **orchestrator-pipeline 两层架构**：19 个 orchestrator 编排 63 个 pipeline skill
...
> **定位**："做得好看且好用" — 视觉设计、交互设计、原型输出、设计规范

**四原则**：
1. 以用户为中心 — Persona 驱动，不假设审美
2. 系统优先 — 先建设计系统，再画页面
3. 无障碍设计 — WCAG 2.1 AA 是硬约束
4. 可交付 — 设计资产必须工程可实现

**Skill 体系**（18 skill = 14 设计 + 4 元）：
- 需求与建议：design-brief / design-recommendation
- 设计系统：design-system / design-system-import / design-system-refactor
- 设计输出：visual-design / interaction-design / wireframe / component-design
- 审查与验证：verify / design-lint / design-review / accessibility-audit
- 交接：design-handoff-spec

**特色机制**：
- **Push-back 反越界**：设计 Agent 有权拒绝并改写 PM 硬编码的 UI 指令
- **反 AI 味**：禁用 Inter/紫色渐变/统一圆角/Lorem ipsum，由 design-lint 机械化检查
- **怀疑驱动对抗审查**：design-review 使用全新上下文子 Agent 做对抗性审查
...
- 文档与 Skill：writing-documentation / writing-skills

**特色机制**：
- **双源 AC 验证**：verify 同时检查工程 AC（`AC-xxx`）和设计 AC（`DAC-xxx`）
- **component-map.json 消费契约**：frontend-implementation 读取组件映射作为唯一真相源
- **熵增检查**：verify 检查文件增长率 / 代码行增长率 / 依赖膨胀 / TODO 积压
- **git hooks**：pre-commit（秘钥/敏感文件/commit-msg 检查）+ pre-push
...
> **定位**："让人用起来" — 内容生产、SEO、用户运营、增长实验

**四原则**：
1. 实验驱动 — 每个动作都有假设和指标
2. 内容优先 — 质量 > 数量，不做内容农场
3. 长期主义 — 不搞黑帽 SEO，不搞虚假流量
4. 数据闭环 — 每个实验都有结论和动作

**Skill 体系**（40 skill = 36 领域 + 4 元）：
- 模块 1 增长战略（5）：nsm-definition / kpi-tree / aarr-diagnosis / growth-loop-design / four-fits-assessment
...
- 模块 6 获客与付费（3）：channel-assessment / landing-page-optimization / cac-analysis
...
- 模块 9 增长复盘（1）：growth-review

**核心产出**：`docs/operations/GROWTH_STRATEGY.md` / `docs/content/` / `docs/seo/` / `docs/experiment/` / `docs/handoff/growth-to-pm.md`

---

### harness-ops（运维基础设施框架）

> **定位**："保驾护航" — 基础设施即代码、自动化部署、监控告警、容灾与故障响应

**SRE 四原则**：
1. 稳定优先 — 不出故障是最高优先级指标
2. 基础设施即代码 — 环境版本控制，一键重建
3. 可观测性 — 日志 / 指标 / 链路，缺一不可
4. 自动化 — 持续消除 toil

**Skill 体系**（32 skill = 28 领域 + 4 元）：
- 模块 1 部署与交付（4）：deployment-pipeline / release-strategy / rollback / deployment-verify
- 模块 2 基础设施（4）：infrastructure-as-code / kubernetes-manifest / helm-management / gitops-sync
- 模块 3 监控与可观测（4）：monitoring-setup / alerting-rules / log-analysis / dashboard-design
- 模块 4 故障响应（4）：incident-detection / root-cause-analysis / incident-mitigation / post-mortem
- 模块 5 安全与合规（4）：secret-management / policy-as-code / security-scan / audit-review
- 模块 6 容量与成本（3）：resource-right-sizing / cost-analysis / capacity-planning
- 模块 7 容灾与备份（3）：backup-management / recovery-drill / disaster-recovery-plan
- 模块 8 运维复盘（2）：ops-review / sla-report

**特色机制**：
- **半自动化架构**：Agent 提议 + 人类审批 + GitOps 执行
- **四种操作原语**（frontmatter `operation_tier` 字段）：
  - `inspect` — 只读巡检，Agent 全自动（13 skill）
  - `propose` — 生成 PR/提案，人类审查后合并（12 skill）
  - `mutate-staging` — 直接在 Staging 执行白名单操作（3 skill）
  - `mutate-prod` — 生产变更，**必须人类审批**
- **GitOps PR 间接执行**：Agent 绝不直接操作生产集群
- **严格 Secret 隔离**：Agent 只操作 Secret 引用，绝不触碰明文值
- **7 层纵深防御**：Dry-run / Canary / 审批门禁 / 速率限制 / 回滚 / 审计 / 爆炸半径
- **7 张知识库表**：故障库 / 根因库 / 部署记录库 / 监控配置库 / IaC 资产库 / 运维模式库 / 踩坑日志
...
框架间通过 `docs/handoff/` 下的契约文档协作。每份文档有明确的**来源框架**和**目标框架**。
...
| 来源 \ 目标 | harness-pm | harness-design | harness-solo | harness-growth | harness-ops |
|-----------|:---:|:---:|:---:|:---:|:---:|
...
**防腐规则**：
- harness-pm 的 PRD 产出 `AC-xxx`（被 UI 指令越界门禁拦截）
- harness-design 的 design-brief 复用 PRD 的 `AC-xxx` 编号，行使 Push-back 清理权
- harness-solo 的 writing-plans 将设计 AC 转为 `DAC-xxx`（D 前缀区分来源）
...
| `.harness/rules/prompt-defense.md` | 提示词注入防御 | ✅ |
...
```
1. AGENTS.md          — 启动时强制读取（唯一强制入口）
2. SOUL.md            — 人格 + 领域价值观
3. constitution.md    — 项目宪法（不可协商原则）
4. skills/INDEX.md    — Skill 索引
5. 对应的 SKILL.md    — 执行任务时按需加载
6. memory/progress.md — session-start 时读取
```
...
**单一 LOOP 硬熔断**：所有框架统一 10 次迭代；超过即停止并请求人类介入。

### 跨框架循环类型对比

| 框架 | 循环类型 | 触发场景 | 最大迭代 |
|------|---------|---------|:---:|
| harness-pm | research | 用户调研 / 市场分析 | 5 |
...
每个框架按领域扩展额外红线：
- **harness-growth**：禁止黑帽 SEO、虚假流量、泄露用户 PII
- **harness-design**：禁止泄露 PII，禁止使用未授权素材
- **harness-ops**：禁止触碰明文 Secret 值，禁止直接操作生产集群

### harness-ops 专属安全机制

- **严格 Secret 隔离**：Agent 只操作 Secret 引用（路径/key 名/CRD），绝不触碰明文值（硬约束，不可协商）
- **7 层纵深防御**：Dry-run / Canary / 审批门禁 / 速率限制 / 回滚 / 审计 / 爆炸半径
- **GitOps PR 间接执行**：Agent 绝不直接操作生产集群；走 PR + 人类审查 + ArgoCD/Flux 同步
...
- ✅ 4 个核心框架独立建成（pm/design/solo/growth 全部完成）
- ✅ 1 个 P0 扩展框架建成（ops，32 skill + 7 workflow，半自动化架构）
- ✅ 契约文档体系打通（pm→design→solo→growth→pm 闭环 + solo→ops→pm 闭环）
- ✅ AC 编号体系跨框架对齐（AC-xxx / DAC-xxx）
- ✅ LOOP 引擎统一规范（state.yaml + 断点续传 + 上限保护）
- ✅ 基础层统一（AGENTS/SOUL/constitution/security/meta skill）
- ✅ 全局深度审计与修复（21 项问题全部解决，生产就绪度 90+）

### 中期演进（v3.0，1-2 个月）

- 📋 harness-data 建成（P1，数据管道框架）
- 📋 契约文档版本化（支持历史追溯，不破坏"只读最新"原则）
- 📋 跨框架循环类型映射说明（design 的 visual-design → solo 的 feature）

### 长期演进（v4.0，3-6 个月，按需）

- 📋 编排层探索（多 Agent 自动调度，非当前目标）
- 📋 共享真相源探索（替代部分契约文档，减少信息损失）
- 📋 harness-qa / harness-security 建成（P2/P3，按需）
...
- [ARCHITECTURE.md](./ARCHITECTURE.md) — 家族架构设计（v2.1，完整版）
...
1. 选择一个框架深入理解（推荐从 harness-solo 开始，最简单）
2. 阅读 [ARCHITECTURE.md](./ARCHITECTURE.md) 理解家族整体设计
3. 阅读对应框架的 AGENTS.md 和 SOUL.md 理解领域价值观
4. 按照 SKILL.md.template 规范创建新 skill
