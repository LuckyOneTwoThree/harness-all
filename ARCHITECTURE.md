...
2. **pm → solo**（`pm-to-solo.md`）：PRD + AC-xxx + 埋点方案 + 关键决策
...
4. skills/INDEX.md    — skill 索引（40 行内）
...
**Skill 体系**（17 skill = 13 设计 + 4 meta）：
...
**已知问题与优化方向**：
- ⚠️ README.md skill 数量标注错误（写 17 工程，实际 16）—— 待修复
- ⚠️ install.sh 未检查 Node.js（webapp-testing 强依赖）—— 待修复
...
| `.harness/skills/INDEX.md` | skill 索引（40 行内） | ✅ |
...
- ✅ harness-pm 的核心交接模板补齐（pm-to-solo-template / solo-to-growth-template）
...
**风险**：PM 产出 PRD → design-brief 解析 → 可能有信息丢失
**缓解**：
