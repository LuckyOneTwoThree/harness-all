...
1. **扫描实际 skill 目录**
   ...
   - 用 Glob 扫描 `.harness/skills/ops/*/SKILL.md`（28 个 ops skill，扁平化组织）
...
3. **一致性对比**
   ...
   | 工作流完整性 | workflows/ 下 7 个 .md 都存在 | 标记"缺失" |
   | ops skill 完整性 | ops/ 下 28 个 skill 目录都有 SKILL.md | 标记"skill 缺失" |
...
5. **检查 ops skill 目录结构**
   ...
   - skill 扁平化组织在 `.harness/skills/ops/` 下（不再按模块分目录）
...
6. **输出健康报告**
   ...
   | 元 skill | ✓ / ✗ | 4/4 存在 |
   | 工作流 | ✓ / ✗ | 7/7 存在 |
   | 工作流 frontmatter | ✓ / ✗ | 7/7 有 workflow_id+name+default_mode |
   | ops skill | ✓ / ✗ | 28/28 skill 存在 |
   | frontmatter | ✓ / ✗ | [问题列表或"无"] |
   ```
