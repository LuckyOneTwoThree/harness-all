...
| substage | Family-wide field with framework-local semantics. Solo uses enum {inline-passed, inline-failed, awaiting-full, full-running, full-passed, full-failed} (see Solo's LOOP.md); PM/Design/Growth/Ops use their own sub-stage values documented in each framework's LOOP.md |
