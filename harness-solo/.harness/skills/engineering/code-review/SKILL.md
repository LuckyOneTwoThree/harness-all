...
- updated spec.md (AC status suffixes: passing ACs → [status: done])
...
- update each passing AC in `loops/specs/<task>/spec.md` from `[status: pending]` to `[status: done]` (per `engineering-pipeline.md` AC ownership and `spec.md.template` status suffix convention)
