...
- **`solo-to-pm.md`**: engineering evidence with cross-framework implications. Publish when ANY of the following occurs:
  ...
  - **Multi-bugfix / refactor accumulation** (low threshold, periodic): when accumulated bugfix/refactor evidence has not been synced to PM for ≥ 1 release cycle, publish a lightweight solo-to-pm summarizing changes so PM's knowledge-base stays current.
